...
impl<T> FastVec<T> {
    ...
    pub fn get(&self, i: usize) -> &T {
        ...
        unsafe{
            let temp = self.ptr_to_data.add(i);
            let value = &*temp;
            return value;
        }
    }
    ...
    pub fn push(&mut self, t: T) {
        if self.len == self.capacity {
            let ptr = MALLOC.malloc((self.capacity * 2) * size_of::<T>()) as *mut T;
            unsafe {
                for i in 0..self.len {
                    ptr::write(ptr.add(i), ptr::read(self.ptr_to_data.add(i)));
                }

                MALLOC.free(self.ptr_to_data as *mut u8);

                self.ptr_to_data = ptr;
                self.capacity = self.capacity * 2;

                ptr::write(ptr.add(self.len), t);
            }
    ...
    pub fn clear(&mut self) {
        for j in 0..self.len {
            unsafe{
                ptr::read(self.ptr_to_data.add(j));
            }
        }        
        MALLOC.free(self.ptr_to_data as *mut u8);
        self.ptr_to_data = null_mut();
